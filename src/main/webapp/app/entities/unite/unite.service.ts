...
export class UniteService {
    ...
    create(unite: IUnite): Observable<EntityResponseType> {
        return this.http.post<IUnite>(this.resourceUrl, unite, { observe: 'response' });
    }

    update(unite: IUnite): Observable<EntityResponseType> {
        return this.http.put<IUnite>(this.resourceUrl, unite, { observe: 'response' });
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<IUnite>(`${this.resourceUrl}/${id}`, { observe: 'response' });
    }

    query(req?: any): Observable<EntityArrayResponseType> {
        const options = createRequestOption(req);
        return this.http.get<IUnite[]>(this.resourceUrl, { params: options, observe: 'response' });
    }
